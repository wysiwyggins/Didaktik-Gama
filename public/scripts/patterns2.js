...
const PATTERN_COLS = 50;
const PATTERN_ROWS = 45;
function preload() {
...
function setup() {
  ...
  createCanvas(PATTERN_COLS * globalVars.TILE_HALF_WIDTH, globalVars.TILE_HALF_HEIGHT * PATTERN_ROWS);
...
function generateBaseAndComplementaryColors() {
  baseColor = color(random(180), random(255), random(255));
...
function drawColorPattern() {
    ...
    for (let y = 0; y < PATTERN_ROWS; y++) {
        for (let x = 0; x < PATTERN_COLS; x++) {
