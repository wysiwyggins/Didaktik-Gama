...
function preload() {
  auto2Spritesheet = loadImage(globalVars.SPRITESHEET_PATH);
...
function setup() {
  ...
  createCanvas(GRID_WIDTH * globalVars.TILE_WIDTH, GRID_HEIGHT * globalVars.TILE_HEIGHT);
...
function drawTile(i, j, val) {
  ...
  let x = (val % SPRITESHEET_COLS) * globalVars.TILE_WIDTH;
  let y = floor(val / SPRITESHEET_COLS) * globalVars.TILE_HEIGHT;
